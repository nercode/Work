...
class InstructionGeneratorService:
	...
	def generate_contexts(self, few_shot: List[Dict], queries: List[Dict], batch_size: int = 10) -> List[Dict]:
		...
		for i in range(0, len(queries), batch_size):
			...
			# 	"topic": f"{TYPE1_PREFIX}{query.get('simplified_query')}" if query.get("type") == 1
			# 	else f"{TYPE2_PREFIX}{query.get('simplified_query')}" if query.get("type") == 2
			# 	else f"{TYPE3_PREFIX}{query.get('simplified_query')}"
			# } for query in batch]
			batch_queries = [{
				"index": query.get("index"),
				"topic": f"{self.prefix_mapping.get(query.get('type'))}{query.get('simplified_query')}"
			} for query in batch]
